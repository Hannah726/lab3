...
public class TupleDesc implements Serializable {
    ...
    public TupleDesc(Type[] typeAr, String[] fieldAr) {
        /*some code goes here*/ 
        if (typeAr.length == 0) {
            throw new IllegalArgumentException("数组至少包含一个元素");
        }
        if (typeAr.length != fieldAr.length) {
            throw new IllegalArgumentException("数组fieldAr长度需要和typeAr长度一致");
        }
    ...
    private TupleDesc(TDItem[] tdItems) {
        if (tdItems == null || tdItems.length == 0) {
            throw new IllegalArgumentException("tdItem数组需要非空且至少包含一个元素");
        }
        this.tdAr = tdItems;
